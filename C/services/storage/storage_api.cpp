 ...
 */
StorageApi::StorageApi(const unsigned short port, const unsigned int threads, const unsigned int poolSize) : m_thread(NULL), readingPlugin(0), streamHandler(0), m_managementClient(NULL)
{
	m_port = port;
	m_threads = threads;
...
static std::atomic<bool> already_running(false);
	...
	try {
		...
		{
			m_managementClient->addAuditEntry("PURGE", "INFORMATION", purged);
		}
		free(purged);
	}
