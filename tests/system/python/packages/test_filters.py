...
""" Test Filters Package System tests:
        ...
        fledge-filter-expression, fledge-filter-python35, fledge-filter-simple-python, fledge-filter-ema filter plugins
"""
...
class TestSimplePython:
    ...
    def setup_class(cls):
        try:
            subprocess.run(["sudo apt install -y fledge-filter-simple-python"], shell=True, check=True)
        except subprocess.CalledProcessError:
            assert False, "installation of packages failed"

    def test_filter_simple_python(self, fledge_url, wait_time):
        add_south_http(fledge_url, self.HTTP_SOUTH_SVC_NAME)
        time.sleep(wait_time * 2)
        verify_south_added(fledge_url, self.HTTP_SOUTH_SVC_NAME)
        ...
        asset_response = verify_asset(fledge_url, ASSET_NAME_SP)
        assert 10 == asset_response["count"]

        reading_resp = verify_readings(fledge_url, ASSET_NAME_SP)
        assert 12.25 == reading_resp["reading"]["sensor"]
    ...
    def test_filter_simple_python_with_updated_content(self, fledge_url, wait_time):
        data = {"code": "reading[b'''sensor'''] = reading[b'''sensor'''] * 10"}
        put_url = "/fledge/category/{}_SP".format(self.HTTP_SOUTH_SVC_NAME)
        put_request(fledge_url, urllib.parse.quote(put_url, safe='?,=,&,/'), data)
    ...
    def test_filter_simple_python_disable_enable(self, fledge_url, wait_time):
        data = {"enable": "false"}
        put_request(fledge_url, urllib.parse.quote("/fledge/category/{}_SP"
                                                    .format(self.HTTP_SOUTH_SVC_NAME), safe='?,=,&,/'), data)
    ...
    def test_filter_simple_python_and_expression(self, fledge_url, wait_time):
        data = {"schedule_name": "{}".format(self.HTTP_SOUTH_SVC_NAME)}
        put_url = "/fledge/schedule/disable"
        put_request(fledge_url, put_url, data)
    ...
    def test_delete_filter_simple_python(self, fledge_url, wait_time):
        data = {"pipeline": ["expr"]}
        put_url = "/fledge/filter/{}/pipeline?allow_duplicates=true&append_filter=false" \
            .format(self.HTTP_SOUTH_SVC_NAME)
        put_request(fledge_url, urllib.parse.quote(put_url, safe='?,=,&,/'), data)
        ...
        conn = http.client.HTTPConnection(fledge_url)
        conn.request("DELETE", '/fledge/filter/SP')
        res = conn.getresponse()
        assert 200 == res.status, "ERROR! Failed to delete filter"
    ...
    def test_filter_simple_python_by_enabling_disabling_south(self, fledge_url, wait_time):
        data = {"schedule_name": "{}".format(self.HTTP_SOUTH_SVC_NAME)}
        put_url = "/fledge/schedule/disable"
        put_request(fledge_url, put_url, data)
...
class TestEma:
    ...
    def setup_class(cls):
        try:
            subprocess.run(["sudo apt install -y fledge-filter-ema"], shell=True, check=True)
        except subprocess.CalledProcessError:
            assert False, "installation of package failed"

    def test_filter_ema(self, fledge_url, retries, wait_time):
        add_south_http(fledge_url, self.HTTP_SOUTH_SVC_NAME)
        time.sleep(wait_time * 2)
        verify_south_added(fledge_url, self.HTTP_SOUTH_SVC_NAME)
        ...
        asset_response = verify_asset(fledge_url, ASSET_NAME_EMA)
        assert 10 == asset_response["count"]

        reading_resp = verify_readings(fledge_url, ASSET_NAME_EMA)
        assert 12.25 == reading_resp["reading"]["sensor"]
    ...
    def test_filter_ema_with_updated_content(self, fledge_url, wait_time):
        data = {"rate": "1"}
        put_url = "/fledge/category/{}_ema".format(self.HTTP_SOUTH_SVC_NAME)
        put_request(fledge_url, urllib.parse.quote(put_url, safe='?,=,&,/'), data)
    ...
    def test_filter_ema_disable_enable(self, fledge_url, wait_time):
        data = {"enable": "false"}
        put_request(fledge_url, urllib.parse.quote("/fledge/category/{}_ema"
                                                    .format(self.HTTP_SOUTH_SVC_NAME), safe='?,=,&,/'), data)
    ...
    def test_filter_ema_and_expression(self, fledge_url, wait_time):
        data = {"schedule_name": "{}".format(self.HTTP_SOUTH_SVC_NAME)}
        put_url = "/fledge/schedule/disable"
        put_request(fledge_url, put_url, data)
    ...
    def test_delete_filter_ema(self, fledge_url, wait_time):
        data = {"pipeline": ["expr"]}
        put_url = "/fledge/filter/{}/pipeline?allow_duplicates=true&append_filter=false" \
            .format(self.HTTP_SOUTH_SVC_NAME)
        put_request(fledge_url, urllib.parse.quote(put_url, safe='?,=,&,/'), data)
        ...
        conn = http.client.HTTPConnection(fledge_url)
        conn.request("DELETE", '/fledge/filter/ema')
        res = conn.getresponse()
        assert 200 == res.status, "ERROR! Failed to delete filter"
    ...
    def test_filter_ema_by_enabling_disabling_south(self, fledge_url, wait_time):
        data = {"schedule_name": "{}".format(self.HTTP_SOUTH_SVC_NAME)}
        put_url = "/fledge/schedule/disable"
        put_request(fledge_url, put_url, data)
