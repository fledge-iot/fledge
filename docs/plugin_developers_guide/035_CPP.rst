...
The *Reading* class and the associated *Datapoint* and *DatapointValue* classes provide the mechanism within C++ classes to maniplated the reading asset data. The public part of the *Reading* class is currently defined as follows;
...
The *Reading* class contains a number of items that are mapped tothe JSON representation of data that is sent to the Fledge storage service and are used by the various services and plugins within Fledge.

  - **Asset Name**: The name of the asset. The asset naem is set in the construtor of the reading and retrieved via the *getAssetName()* method.

  - **Timestamp**: The timestamp when the reading was first seen within Fledge.

  - **User Timestamp**: The tiemstamp for the actual data in the reading. This may differ from the value of Timestamp if the device itself is able to supply a timestamp value.
...
Closely associated with the *Datapoint* is the *DatapointValue* which uses a tagged union to store the values. The public defintion of the *DatapointValue*  is as follows;
...
Although *ConfigCategory* is a complex class, only a few of the methods are commonly used within a plugin
  ...
  - **isDouble:** - tests if a configuration item is valid to be repesented as a double
...
.. code-block:: C
  ...
  logger->error("An error has occured within the plugin processing");


It is then possible to log messages at one of five different log levels; *debug*, *info*, *warn*, *error* or *fatal*. Messages may be logged using standard printf formatting strings. The public defintion of the *Logger* class is as follows;
...
The various log levels should be used as follows;
  ...
  - **debug**: should be used to output messages that are relavent only to a programmgn that is debugging the plugin.
  - **info**: should be used for information that is measningful to the end users, but should not normally be logged.
  ...
  - **fatal**: should be used for conditions that cause the plugin to fail processing permenantly, possibly requiring a restart of the microservce in order to resolve.
