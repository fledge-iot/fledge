...
class SendingProcess:
    ...
    # FIXME:
    _MEMORY_BUFFER_MAIN_SLEEP = 1
    _MEMORY_BUFFER_FETCH_SLEEP = 0.1
    _MEMORY_BUFFER_SEND_SLEEP = 0.1

    _memory_buffer_fetch_idx = 0
    _memory_buffer_send_idx = 0
    _memory_buffer_max_size = 5
    _memory_buffer = [None, None, None, None, None, ]
    ...
    async def task_fetch_data(self, stream_id):
        ...
        while self._run_task_fetch_data:
            ...
            if self._memory_buffer_fetch_idx < self._memory_buffer_max_size:

                if self._memory_buffer[self._memory_buffer_fetch_idx] == None:
                    ...
                    # FIXME:self._memory_buffer[self._memory_buffer_fetch_idx] = copy.deepcopy(data_to_send)
                    self._memory_buffer[self._memory_buffer_fetch_idx] = data_to_send
                    last_object_id = data_to_send[self._config['blockSize'] - 1]['id']

                    self._memory_buffer_fetch_idx = self._memory_buffer_fetch_idx +1
                else:
                    await asyncio.sleep(self._MEMORY_BUFFER_FETCH_SLEEP)
    ...
    async def task_send_data(self, stream_id):
        ...
        while self._run_task_send_data:
            ...
            if self._memory_buffer_send_idx < self._memory_buffer_max_size:

                if self._memory_buffer[self._memory_buffer_send_idx] != None:
                    ...
                    self._memory_buffer_send_idx = self._memory_buffer_send_idx + 1

                else:
                    await asyncio.sleep(self._MEMORY_BUFFER_SEND_SLEEP)
    ...
    async def send_data(self, stream_id):
        ...
        try:
            ...
            while elapsed_seconds < self._config['duration']:
                ...
                try:
                    await asyncio.sleep(self._MEMORY_BUFFER_MAIN_SLEEP)
                    # FIXME:
    ...
    def _update_statistics(self, num_sent, stream_id):
        ...
        try:
            ...
            # FIXME:
            #asyncio.get_event_loop().run_until_complete(_stats.update(key, num_sent))
