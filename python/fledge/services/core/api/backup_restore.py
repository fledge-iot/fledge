...
async def get_backup_download(request):
    ...
    try:
        ...
        # Download the backup file using the backward compatible way or the current one
        dummy, file_extension = os.path.splitext(source)
        _logger.debug("get_backup_download - file_extension :{}: -".format(file_extension) )
        ...
        if file_extension == ".db":
            ...
            _logger.debug("get_backup_download - backward - gz_path :{}:".format (gz_path) )

        else:
            gz_path = Path(source)
            _logger.debug("get_backup_download - new - gz_path :{}:".format (gz_path) )
    ...
    try:
        ...
        if any((item.startswith(backup_prefix) and item.endswith(valid_extensions)) for item in tar_file_names):
            ...
            if any((item.startswith("etc") and item.endswith("etc")) for item in tar_file_names):
                ...
                _logger.debug("upload_backup - new - source :{}: cmd :{}:cmd :{}:".format (source, cmd, backup_file_name) )
                ...
                _logger.debug("upload_backup - backward - source :{}: cmd :{}:cmd :{}:".format (source, cmd, backup_file_name) )
