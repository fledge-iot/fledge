...
async def update_password(request):
    ...
    if new_password and not isinstance(new_password, str):
        err_msg = "New password should not be provided in a non-string format."
...
async def validate_password(password) -> str:
    ...
    if len(password) < int(min_chars):
        message = "Password should have a minimum length of {} characters.".format(min_chars)
    if len(password) > int(max_chars):
        message = "Password should have a maximum length of {} characters.".format(max_chars)
    if not message:
