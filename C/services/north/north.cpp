...
int makeDaemon()
{
  ...
  if (dup(0) == -1)
  {
  } // stdout	GCC bug 66425 produces warning
  if (dup(0) == -1)
  {
  } // stderr	GCC bug 66425 produces warning
  return 0;
}
  ...
  for (int i = 0; i < size; i++)
  {
    ...
    if (dladdr(array[i], &info) && info.dli_sname)
    {
      ...
      snprintf(buf, sizeof(buf), "%-3d %*p %s + %zd---------",
               ...
               status == 0 ? demangled : info.dli_sname == 0 ? messages[i]
                                                             : info.dli_sname,
 ...
 */
NorthService::NorthService(const string &myName, const string &token) : m_dataLoad(NULL),
                                                                        m_dataSender(NULL),
                                                                        northPlugin(NULL),
                                                                        m_assetTracker(NULL),
                                                                        m_shutdown(false),
...
void NorthService::addConfigDefaults(DefaultConfigCategory &defaultConfig)
{
  ...
  defaultConfig.setItemAttribute("assets",
                                 ConfigCategory::VALIDITY_ATTR, "logLevel == \"info\"");
