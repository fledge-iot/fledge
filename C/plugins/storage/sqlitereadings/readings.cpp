 ...
 */
int rowidReadingCallback(void *data,
			 int nCols,
			 char **colValues,
			 char **colNames)
{
...
bool Connection::aggregateQuery(const Value& payload, string& resultSet)
{
	...
	sql.append("json_each.key AS x, json_each.value AS theval FROM readings, json_each(reading) ");
...
int Connection::readingStream(ReadingStream **readings, bool commit)
{
	...
	const char *sql_cmd = "INSERT INTO readings ( asset_code, reading, user_ts ) VALUES  (?,?,?)";
...
int sleep_time_ms = 0;
	...
	const char *sql_cmd="INSERT INTO readings ( user_ts, asset_code, reading ) VALUES  (?,?,?)";
...
int retrieve;
	...
	// SQL command to extract the data from the readings
...
int blocks = 0;
	...
	if (age == 0)
	{
		 ...
		 */
		SQLBuffer oldest;
		oldest.append("SELECT (strftime('%s','now', 'utc') - strftime('%s', MIN(user_ts)))/360 FROM readings where rowid <= ");
...

	{
		...
		while (l <= r)
		{
			...
			SQLBuffer sqlBuffer;
			sqlBuffer.append("select id from readings where rowid = ");
			sqlBuffer.append(m);
	...
	if ((flags & 0x01) == 0)
	{
		...
		int lastPurgedId;
		SQLBuffer idBuffer;
		idBuffer.append("select id from readings where rowid = ");
	...
	while (rowidMin < rowidLimit)
	{
		...
		SQLBuffer sql;
		sql.append("DELETE FROM readings WHERE rowid <= ");
		sql.append(rowidMin);
		sql.append(';');
		const char *query = sql.coalesce();
