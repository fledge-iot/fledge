...
/**
 * Constructor for the LoggerLinux class.
 *
 * @param application	The application name
 */
LoggerLinux::LoggerLinux(const string& application)
{
	m_runWorker = true;
	m_workerThread = NULL;
...
/**
 * Destructor for the LoggerLinux class.
 */
LoggerLinux::~LoggerLinux()
{
 ...
 */
void LoggerLinux::sendToUdpSink(const std::string& msg) 
{
	if (m_UdpSockFD >= 0) 
	{
...
/**
 * Return the singleton instance of the LoggerLinux class.
 */
Logger *Logger::getLogger()
{
	if (!instance)
	{
		// Any service should have already created the LoggerLinux
		// for the service. If not then create the default LoggerLinux
		// and clearly identify this. We should ideally avoid
		// the use of a default as this will not identify the
		// source of the log message.
		instance = (Logger *)new LoggerLinux("(default)");
	}
...
{
	return (Logger *)new LoggerLinux(application);
}
 ...
 */
void LoggerLinux::setMinLevel(const string& level)
{
 ...
 */
bool LoggerLinux::registerInterceptor(LogLevel level, LogInterceptor callback, void* userData)
{
	...
	{
		m_workerThread = new std::thread(&LoggerLinux::workerThread, this);
	}
 ...
 */
bool LoggerLinux::unregisterInterceptor(LogLevel level, LogInterceptor callback)
{
 ...
 */
void LoggerLinux::executeInterceptor(LogLevel level, const std::string& message)
{
	std::lock_guard<mutex> lock(m_interceptorMapMutex);
	auto range = m_interceptors.equal_range(level);
	for (auto it = range.first; it != range.second; ++it)
 ...
 */
void LoggerLinux::debug(const string& msg, ...)
{
	va_list args;
	va_start(args, msg);
 ...
 */
void LoggerLinux::printLongString(const string& s, LogLevel level)
{
 ...
 * @param ...		The variable arguments required by the printf format
 */
void LoggerLinux::info(const std::string& msg, ...)
{
	va_list args;
	va_start(args, msg);
 ...
 * @param ...		The variable arguments required by the printf format
 */
void LoggerLinux::warn(const string& msg, ...)
{
	va_list args;
	va_start(args, msg);
 ...
 * @param ...		The variable arguments required by the printf format
 */
void LoggerLinux::error(const string& msg, ...)
{
	va_list args;
	va_start(args, msg);
 ...
 * @param ...		The variable arguments required by the printf format
 */
void LoggerLinux::fatal(const string& msg, ...)
{
	va_list args;
	va_start(args, msg);
 ...
 * @param ...		The variable arguments required by the printf format
 */
void LoggerLinux::log(int sysLogLvl, const char * lvlName, LogLevel appLogLvl, const std::string& msg, va_list args)
{
