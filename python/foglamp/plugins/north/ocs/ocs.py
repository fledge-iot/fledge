...
def plugin_send(data, raw_data, stream_id):
    ...
    ocs_north = OCSNorthPlugin(data['sending_process_instance'])

    try:
        is_data_available, new_position, num_sent = ocs_north._transform_in_memory_data(data_to_send, raw_data)
        if is_data_available:
            ocs_north._create_omf_objects(raw_data, config_category_name, type_id)
            try:
                ocs_north._send_in_memory_data_to_picromf("Data", data_to_send)
            except Exception as ex:
                # Forces the recreation of PIServer's objects on the first error occurred
                if _recreate_omf_objects:
                    ocs_north._deleted_omf_types_already_created(config_category_name, type_id)
...
class OCSNorthPlugin(object):
    ...
    def _create_omf_type_automatic(self, asset_info):
        ...
        for item in asset_data:
            ...
            _logger.debug(
                "func |{func}| - item_type |{type}| - formatInteger |{int}| - formatNumber |{float}| ".format(
                            ...
                            type=item_type,
                            int=_config['formatInteger'],
                            float=_config['formatNumber']))
            ...
            if item_type == "integer":
                ...
                omf_type[typename][1]["properties"][item] = {"type": item_type,
                                                             "format": _config['formatInteger']}
            elif item_type == "number":
                omf_type[typename][1]["properties"][item] = {"type": item_type,
                                                             "format": _config['formatNumber']}
            else:
                omf_type[typename][1]["properties"][item] = {"type": item_type}
        ...
        self._send_in_memory_data_to_picromf("Type", omf_type[typename])
    ...
    def _create_omf_object_links(self, asset_code, typename, omf_type):
        ...
        if _log_debug_level == 3:
            ...
            _logger.debug("_create_omf_object_links - asset_code |{0}| - link_data |{1}| ".format(asset_code,
                                                                                                  str(link_data)))
        self._send_in_memory_data_to_picromf("Container", containers)
        self._send_in_memory_data_to_picromf("Data", static_data)
        self._send_in_memory_data_to_picromf("Data", link_data)
        return
    ...
    @_performance_log
    def _send_in_memory_data_to_picromf(self, message_type, omf_data):
    ...
    @_performance_log
    def _transform_in_memory_data(self, data_to_send, raw_data):
        ...
        try:
            for row in raw_data:
                ...
                try:
                    ...
                    num_unsent += 1
                    _logger.warning(plugin_common.MESSAGES_LIST["e000023"].format(e))
        except Exception:
            _logger.error(plugin_common.MESSAGES_LIST["e000021"])
            raise
    ...
    def _transform_in_memory_row(self, data_to_send, row, target_stream_id):
        ...
        try:
            ...
            if data_available:
                ...
                _logger.warning(plugin_common.MESSAGES_LIST["e000020"])
        except Exception:
            _logger.error(plugin_common.MESSAGES_LIST["e000022"])
            raise
