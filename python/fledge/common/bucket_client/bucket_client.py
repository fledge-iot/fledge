...
from fledge.common.bucket_client.exceptions import *
from fledge.common.bucket_client.model_query_builder import ModelQueryBuilder
from fledge.common.bucket_client.utils import Utils
...
class BucketClientAsync(AbstractStorage):
    ...
    def service(self, svc):
        if not isinstance(svc, ServiceRecord):
            w_msg = 'Bucket should be a valid Fledge micro-service instance'
            _LOGGER.warning(w_msg)
            raise InvalidServiceInstance

        if not getattr(svc, "_type") == "Bucket":
            w_msg = 'Bucket should be a valid *Storage* micro-service instance'
            _LOGGER.warning(w_msg)
            raise InvalidServiceInstance

        self.__service = svc

    def _get_bucket_service(self, host, port):
        """ get Storage service """

        conn = http.client.HTTPConnection("{0}:{1}".format(host, port))
        # TODO: need to set http / https based on service protocol

        conn.request('GET', url='/fledge/service?name=Fledge%20Bucket')
        r = conn.getresponse()
    ...
    def connect(self, core_management_host, core_management_port):
        svc = self._get_bucket_service(host=core_management_host, port=core_management_port)
        if len(svc) == 0:
            raise InvalidServiceInstance
    ...
    async def query_models(self, attributes):
       """ Use the bucket service API to get list of buckets with matching attribute values

        Here bucket API gets called to get matching buckets based on provided attribute list

        :param attributes: List of key/value tuples to use for attribute based bucket search
        :return: a JSON object containing information about matching buckets
        """
        ...
        async with aiohttp.ClientSession() as session:
            async with session.put(url, data=payload) as resp:
                ...
                if status_code not in range(200, 209):
                    ...
                    raise BucketServerError(code=resp.status, reason=resp.reason, error=jdoc)
