...
class TestAuthMandatory:
    ...
    async def test_bad_update_user(self, client, mocker):
        ...
        with patch.object(User.Objects, 'get', side_effect=[_se1, Exception(msg)]):
    ...
    async def test_bad_role_in_update_user(self, client, mocker):
        ...
        # Changed in version 3.8: patch() now returns an AsyncMock if the target is an async function.
        if sys.version_info.major == 3 and sys.version_info.minor >= 8:
            _rv = await mock_coro(False)
            _se1 = await mock_coro(u1)
        else:
            _rv = asyncio.ensure_future(mock_coro(False))
            _se1 = asyncio.ensure_future(mock_coro(u1))
    ...
    async def test_update_password_with_invalid_current_password(self, client):
        ...
        patch_logger_info.assert_called_once_with('Received %s request for %s', 'PUT', '/fledge/user/{}/password'.format(uid))
    ...
    async def test_update_password_exceptions(self, client, exception_name, status_code, msg):
        ...
        # Changed in version 3.8: patch() now returns an AsyncMock if the target is an async function.
        if sys.version_info.major == 3 and sys.version_info.minor >= 8:
            _rv = await mock_coro(2)
        else:
            _rv = asyncio.ensure_future(mock_coro(2))
    ...
    async def test_delete_own_account(self, client, mocker):
        ...
        patch_logger_info, patch_validate_token, patch_refresh_token, patch_user_get = await self.auth_token_fixture(mocker, is_admin=False)
        
        # Changed in version 3.8: patch() now returns an AsyncMock if the target is an async function.
        if sys.version_info.major == 3 and sys.version_info.minor >= 8:
            _rv = await mock_coro([{'id': '2'}])
        else:
            _rv = asyncio.ensure_future(mock_coro([{'id': '2'}]))
