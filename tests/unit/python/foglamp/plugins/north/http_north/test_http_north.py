...
async def test_plugin_send():
    ...
    with patch.object(http_north.http_north, '_send_payloads', return_value=async_mock()) as patch_send_payload:
        is_data_sent, new_last_object_id, num_sent = http_north.plugin_send(data=data, payload=payload, stream_id=3)
        assert (False, 0, 0) == (is_data_sent, new_last_object_id, num_sent)
...
async def test_plugin_shutdown(loop):
    ...
    with patch.object(http_north.http_north, 'cancel_tasks', return_value=async_mock()) as patch_cancel:
