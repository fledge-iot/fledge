...
class TestPluginUpdate:
    ...
    async def test_package_already_in_progress(self, client):
        ...
        plugin_installed_dir = 'http_north'
        pkg_name = "fledge-north-http-north"
        ...
        with patch.object(connect, 'get_storage_async', return_value=storage_client_mock):
            with patch.object(storage_client_mock, 'query_tbl_with_payload',
                ...
                resp = await client.put('/fledge/plugins/north/{}/update'.format(plugin_installed_dir), data=None)
    ...
    async def test_plugin_not_found(self, client):
        async def async_mock(return_value):
            return return_value

        plugin_type = "south"
        plugin_installed_dir = "Random"
        pkg_name = "fledge-{}-random".format(plugin_type)
        payload = {"return": ["status"], "where": {"column": "action", "condition": "=", "value": "update",
                                                   "and": {"column": "name", "condition": "=", "value": pkg_name}}}
        plugin_installed = [{"name": "sinusoid", "type": plugin_type, "description": "Sinusoid Poll Plugin",
                             "version": "1.8.1", "installedDirectory": "{}/sinusoid".format(plugin_type),
                             "packageName": "fledge-{}-sinusoid".format(plugin_type)}]
        storage_client_mock = MagicMock(StorageClientAsync)
        with patch.object(connect, 'get_storage_async', return_value=storage_client_mock):
            with patch.object(storage_client_mock, 'query_tbl_with_payload',
                ...
                with patch.object(PluginDiscovery, 'get_plugins_installed', return_value=plugin_installed
                    ...
                    resp = await client.put('/fledge/plugins/{}/{}/update'.format(plugin_type, plugin_installed_dir),
                                            data=None)
                    assert 404 == resp.status
                    assert "'{} plugin is not yet installed. So update is not possible.'".format(
                        plugin_installed_dir) == resp.reason
                plugin_installed_patch.assert_called_once_with(plugin_type, False)
            args, kwargs = query_tbl_patch.call_args_list[0]
            assert 'packages' == args[0]
            assert payload == json.loads(args[1])

    async def test_plugin_update_when_not_in_use(self, client):
        async def async_mock(return_value):
            return return_value

        plugin_type = "south"
        plugin_installed_dirname = "Random"
        pkg_name = "fledge-{}-random".format(plugin_type)
        payload = {"return": ["status"], "where": {"column": "action", "condition": "=", "value": "update",
                                                   "and": {"column": "name", "condition": "=", "value": pkg_name}}}
        plugin_installed = [{"name": plugin_installed_dirname, "type": plugin_type,
                             "description": "Random data generation", "version": "1.8.1",
                             "installedDirectory": "{}/{}".format(plugin_type, plugin_installed_dirname),
        ...
        with patch.object(connect, 'get_storage_async', return_value=storage_client_mock):
            with patch.object(storage_client_mock, 'query_tbl_with_payload',
                ...
                with patch.object(PluginDiscovery, 'get_plugins_installed', return_value=plugin_installed
                    ...
                    plugin_tracked_patch.assert_called_once_with(plugin_type)
                plugin_installed_patch.assert_called_once_with(plugin_type, False)
            args, kwargs = query_tbl_patch.call_args_list[0]
            assert 'packages' == args[0]
            assert payload == json.loads(args[1])

    async def test_plugin_update_when_in_use(self, client):
        async def async_mock(return_value):
            return return_value
    ...
    async def test_notify_plugin_update_when_not_in_use(self, client):
        async def async_mock(return_value):
            return return_value

        plugin_type = "notify"
        plugin_type_installed_dir = "notificationDelivery"
        plugin_installed_dirname = "Telegram"
        pkg_name = "fledge-{}-telegram".format(plugin_type)
        payload = {"return": ["status"], "where": {"column": "action", "condition": "=", "value": "update",
                                                   "and": {"column": "name", "condition": "=", "value": pkg_name}}}
        plugin_installed = [{"name": plugin_installed_dirname, "type": plugin_type,
                             "description": "Telegram notification delivery C plugin", "version": "1.8.1",
    ...
    async def test_notify_plugin_update_when_in_use(self, client):
        async def async_mock(return_value):
            return return_value
        ...
        read_cat_val = {"name": {"description": "The name of this notification", "type": "string",
                        ...
                        "rule": {"description": "Rule to evaluate", "type": "string", "default": plugin_installed_dirname,
                                 "value": plugin_installed_dirname},
