...
class ConfigurationManager(ConfigurationManagerSingleton):
    ...
    async def search_for_ACL_recursive_from_cat_name(self, cat_name):
            ...
            for item_name, item_info in row["value"].items():
                try:
                    if item_info["type"] == "ACL":
                        return True, item_name, cat_name
                except KeyError:
                    continue
        ...
        for row in child_results['rows']:
            res, config_item_name, found_cat_name = await self.search_for_ACL_recursive_from_cat_name(row["child"])
            if res:
                return True, config_item_name, found_cat_name
    ...
    async def create_category(self, category_name, category_value, category_description='', keep_original_items=False, display_name=None):
        ...
        try:
            ...
            if is_acl:
                await self._acl_handler.handle_create_for_acl_config_item(found_cat_name, config_item)
        except:
            _logger.exception(
    ...
    async def delete_child_category(self, category_name, child_category):
        ...
        is_acl_parent, parent_config_item, found_cat_name_parent = await  \
            self.search_for_ACL_recursive_from_cat_name(category_name)
        _logger.debug("IF acl is {} parent".format(is_acl_parent))
        is_acl_child, child_config_item, found_cat_name_child = await \
            self.search_for_ACL_recursive_from_cat_name(child_category)
        _logger.debug("IF acl is {} child".format(is_acl_child))
        if is_acl_parent:
            await self._acl_handler.handle_delete_for_acl_config_item(found_cat_name_parent, parent_config_item)
        if is_acl_child:
            await self._acl_handler.handle_delete_for_acl_config_item(found_cat_name_child, child_config_item)
    ...
    async def delete_parent_category(self, category_name):
        ...
        _logger.debug("IF acl is {}".format(is_acl))
        if is_acl:
            await self._acl_handler.handle_delete_for_acl_config_item(found_cat_name, config_item)
