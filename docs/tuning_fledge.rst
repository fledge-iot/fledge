...
The buffering subsystem within the south service will buffer readings in the south as they arrive. It checks the time difference between the oldest buffered reading and the current time to see if the maximum latency setting is about to be exceeded. If it is it will send the buffered data. If it is not then it will check the number of readings buffered and if that is about to exceed the maximum allowed number of buffered readings it will then send all the buffered readings. No further checks are done until the next reading arrives.

Therefore, if readings do not arrive very frequently, or the south plugin is asynchronous and data arrives sporadically, then it may not check the buffer status for more than the maximum configured latency period. This results in that maximum latency being exceeded. When this occurs a warning message will be logged in the system logs.

It is recommended in these circumstances to disable or severely limit the buffering in the south service. This will result in less efficient interactions wit the storage system, but these will be infrequent due to the infrequent nature of data arrival.
