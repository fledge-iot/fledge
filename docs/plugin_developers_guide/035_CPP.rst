...
The *Reading* class contains a number of items that are mapped to the JSON representation of data that is sent to the Fledge storage service and are used by the various services and plugins within Fledge.
  ...
  - **Asset Name**: The name of the asset. The asset name is set in the construtor of the reading and retrieved via the *getAssetName()* method.
...
The various log levels should be used as follows;
  ...
  - **debug**: should be used to output messages that are relavent only to a programmer that is debugging the plugin.
  ...
  - **fatal**: should be used for conditions that cause the plugin to fail processing permenantly, possibly requiring a restart of the microservice in order to resolve.
