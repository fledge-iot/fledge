...
async def get_snapshot(request):
    ...
    :Example:
        ...
        curl -X GET http://localhost:8081/foglamp/snapshot/plugins -H "authorization: <token>" 
    """
...
async def post_snapshot(request):
    ...
    :Example:
        ...
        When auth is mandatory:
        curl -X POST http://localhost:8081/foglamp/snapshot/plugins -H "authorization: <token>" 
    """
    try:
        snapshot_dir = _get_snapshot_dir()
...
async def put_snapshot(request):
    ...
    :Example:
        ...
        When auth is mandatory:
        curl -X PUT http://localhost:8081/foglamp/snapshot/plugins/1554204238 -H "authorization: <token>" 
    """
    try:
...
async def delete_snapshot(request):
    ...
    :Example:
        ...
        When auth is mandatory:
        curl -X DELETE http://localhost:8081/foglamp/snapshot/plugins/1554204238 -H "authorization: <token>" 
    """
    try:
        snapshot_id = request.match_info.get('id', None)
        snapshot_name = "snapshot-plugin-{}.tar.gz".format(snapshot_id)
...
def _get_snapshot_dir():
    if _FOGLAMP_DATA:
        ...
        snapshot_dir = os.path.expanduser(
            _FOGLAMP_ROOT + '/data/snapshots/plugins')
    return snapshot_dir
